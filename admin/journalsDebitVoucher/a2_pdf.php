...
if ($journals->num_rows > 0){
	...
	while($row = $journals->fetch_assoc()){
		...
		if ($journals_details->num_rows > 0){
			...
			if ($exp_group > 2){
				$journals_total = $conn->query("SELECT sum(je.gross_amt) as Crt_total from `journal_entries` je inner join payee p on p.id = je.payee_name WHERE je.adjusted is null and journal_date <= '$end' and dli_type = '$d_type' and je.exp_group = '$exp_group' and je.exp_type=1 and p.iufr_flag = 1 and p.id = '$payee_id' group by p.id;");
			  ...
			  if (($exp_group == 1) && ($d_type == 'DLI')) {
				...
				$data .= '<td class="cl9" style ="padding: 3px;">'.$inv_date.'</td>';
				 if ($exp_group == 2){
					$journals_2 = $conn->query("SELECT sum(je.gross_amt) as Crt_2 from `journal_entries` je inner join payee p on p.id = je.payee_name WHERE adjusted is null and journal_date >= '$start' and journal_date <= '$end' and je.exp_group = '$exp_group' and je.exp_type=1 and p.iufr_flag = 1 and p.id = '$payee_id' group by p.id;");
