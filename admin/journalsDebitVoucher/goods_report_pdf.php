...
$title = "";
//$title .= 'Group Name: <b>'.$t_row_d['payee_name'].'</b>


$t_row = $conn->query("SELECT exp_name FROM `exp_group` where id = '$account_id'");

while($t_row_d = $t_row->fetch_assoc()){
	$title .= 'Group Name: <b>'.$t_row_d['exp_name'].'</b>';
}
...
/*
if ($d_type != 'Combined')
	$details = $conn->query("SELECT journal_date, description, gross_amt FROM `journal_entries` je inner join payee p on p.id = je.payee_name where je.payee_name = '$account_id' and journal_date >= '$start' and journal_date <= '$end' and dli_type = '$d_type' order by journal_date ASC");
else
...
$details = $conn->query("SELECT journal_date, description, gross_amt FROM `journal_entries` where exp_group = '$account_id' and journal_date >= '$start' and journal_date <= '$end' and (payee_name <> 5 and payee_name <> 13) order by journal_date ASC");
