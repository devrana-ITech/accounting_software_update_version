...
$data_dr .= '<tr>';
$data_dr .= '<td style="background-color: #f3e3fa; font-size: 10px;" colspan="3"><b>Total<b></td>';
if ($bank_total_dr != 0)
    $data_dr .= '<td style="background-color: #f3e3fa; font-size: 10px;" class="cl5"><b>'.number_format($bank_total_dr, 2).'</b></td>';
else
     $data_dr .= '<td style="background-color: #f3e3fa;" class="cl5"><b>--</b></td>';
if ($cash_total_dr != 0)
	  $data_dr .= '<td style="background-color: #f3e3fa; font-size: 10px;" class="cl5"><b>'.number_format($cash_total_dr, 2).'</b></td></tr>';
else
     $data_dr .= '<td style="background-color: #f3e3fa;" class="cl5"><b>--</b></td></tr>';
...
$numrows = $conn->query("SELECT je.journal_date, jt.journal_id, jt.group_id, je.description, al.name, je.voucher_number, jt.amount, jt.account_id, jt.group_id, je.payee_name FROM `journal_entries` je inner join `journal_items` jt on je.id = jt.journal_id inner join account_list al on al.id = jt.account_id WHERE dli_type = 'Non-DLI' and account_id >= 51 and account_id <= 52 and journal_date >= '$start' and journal_date <= '$end' order by journal_date asc;")->num_rows;
	  ...
	  if ($numrows > 0){	
		while($row = $journals->fetch_assoc()){
			...
			if (($row['debit_count'] > 1) && ($row['credit_count'] == 1) && ($row['group_id'] == 4)) {
				...
				while($row_j = $journals_item->fetch_assoc()){
					  ...
					  $data_dv .= '<td class="cl1">'.$row_j['name'].'</td>';
		...
		if (($bank_total_cr != 0) && ($cash_total_cr != 0)) {
			...
			$data_dv .= '<td class="cl5" style="padding: 3px;">'.number_format($cash_total_cr, 2).'</td></tr>';
			$counter_row_credit = $counter_row_credit + 1;
		...
		if (($bank_balance != 0) && ($cash_balance != 0)) {
			...
			$data_dv .= '<td class="cl5" style="padding: 3px;">'.number_format($cash_balance, 2).'</td></tr>';
			$counter_row_credit = $counter_row_credit + 1;
...
$data_cr .= '<tr>';
$data_cr .= '<td style="background-color: #b7f7db; font-size: 10px;" colspan="3"><b>Total<b></td>';
if (($bank_total_cr + $bank_balance) != 0)
  $data_cr .= '<td style="background-color: #b7f7db; font-size: 10px;" class="cl5"><b>'.number_format(($bank_total_cr + $bank_balance), 2).'</b></td>';
else
    $data_cr .= '<td style="background-color: #b7f7db;" class="cl5"><b>--</b></td>';
if (($cash_total_cr + $cash_balance) != 0)
  $data_cr .= '<td style="background-color: #b7f7db; font-size: 10px;" class="cl5"><b>'.number_format(($cash_total_cr + $cash_balance), 2).'</b></td></tr>';
else
    $data_cr .= '<td style="background-color: #b7f7db;" class="cl5"><b>--</b></td></tr>';


$header = '<!--mpdf
<htmlpageheader name="letterheader">
		<div style="font-size: 10pt; text-align: center; padding-top: 1mm; font-family: nikosh; ">
		<h3 style="margin-bottom: 2px;">==== Cash Book (Non-DLI)====</h3>
