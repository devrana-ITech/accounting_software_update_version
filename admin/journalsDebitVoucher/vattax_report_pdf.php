...
if (($account_id == 89) || ($account_id == 96))
	$details = $conn->query("SELECT journal_date, vat_deduction, it_deduction, (vat_deduction+it_deduction) as linetotal FROM `journal_entries` where (payee_name = 89 or payee_name = 96) and journal_date >= '$start' and journal_date <= '$end' and (vat_deduction <> 0 or it_deduction <> 0) order by journal_date ASC");
else
	$details = $conn->query("SELECT journal_date, vat_deduction, it_deduction, (vat_deduction+it_deduction) as linetotal FROM `journal_entries` where payee_name = '$account_id' and journal_date >= '$start' and journal_date <= '$end' and (vat_deduction <> 0 or it_deduction <> 0) order by journal_date ASC");
...
while($row_d = $details->fetch_assoc()){
	...
	.'<td style="text-align: center; width: 22%; font-size: 12px;">'.date("d-m-Y", strtotime($row_d['journal_date'])).'</td>'
	.'<td style="text-align: right; width: 22%; font-size: 12px;">'.number_format($row_d['vat_deduction'], 2).'</td>'
	.'<td style="text-align: right; width: 22%; font-size: 12px;">'.number_format($row_d['it_deduction'], 2).'</td>'
	.'<td style="text-align: right; width: 24%; font-size: 12px;">'.number_format($row_d['linetotal'], 2).'</td></tr>';
	
}
$data .= '<tr><td style="text-align: left; font-size: 12px; width: 32%;" colspan="2"><b>Total</b></td>
<td style="text-align: right; font-size: 12px; width: 22%;"><b>'.number_format($vat, 2).'</b></td><td style="text-align: right; font-size: 12px; width: 22%;"><b>'.number_format($it, 2).'</b></td><td style="text-align: right; font-size: 12px; width: 24%;"><b>'.number_format($net_amt,2).'</b></td></tr>';
...
<htmlpageheader name="letterheader">
		...
		<h4 style="margin-bottom: 2px;">VAT and Income Tax Statement</h4>
...
table, th, td {
  ...
  <table>
	...
	<tr>
	  ...
	  <th style="background-color: #f3e3fa; text-align: center; width: 22%; font-size: 12px;"><b>Transaction Date</b></th>
	  <th style="background-color: #f3e3fa; text-align: right; width: 22%; font-size: 12px;"><b>VAT</b></th>
	  <th style="background-color: #f3e3fa; text-align: right; width: 22%; font-size: 12px;"><b>Income Tax</b></th>
	  <th style="background-color: #f3e3fa; text-align: right; width: 24%; font-size: 12px;"><b>Total</b></th>
	</tr>
	</thead>
