...
while($row = $journals->fetch_assoc()){
	   if ($row['Crt'] != 0) {
			...
			if ($row['name'] == 'Bank') {
				...
				$data .= '<td class="cl6" style="padding: 3px; text-align: right">--</td></tr>';
				$bank_total_dr += $row['Crt'];
			}
			if ($row['name'] == 'Cash') {
				$data .= '<td class="cl4" style="padding: 3px; text-align: right;">--</td>';
				$data .= '<td class="cl6" style="padding: 3px; text-align: right;">'.format_num($row['Crt']).'</td></tr>';
	...
	while($row = $journals->fetch_assoc()){
			...
			if (($row['debit_count'] > 1) && ($row['credit_count'] == 1) && ($row['group_id'] == 1)) {
				...
				while($row_j = $journals_item->fetch_assoc()){
					  ...
					  if ($row['name'] == 'Cash') {
							$data .= '<td class="cl4" style="padding: 3px; text-align: right;">--</td>';
							$data .= '<td class="cl6" style="padding: 3px; text-align: right;">'.format_num($dbt_amt).'</td></tr>';
			...
			if (($row['debit_count'] == 1) && ($row['credit_count'] > 1) && ($row['group_id'] == 1)) {
				...
				while($row_j = $journals_item->fetch_assoc()){
					  ...
					  if ($row['name'] == 'Cash') {
							$data .= '<td class="cl4" style="padding: 3px; text-align: right;">--</td>';
							$data .= '<td class="cl6" style="padding: 3px; text-align: right;">'.format_num($row_j['amount']).'</td></tr>';
...
$data_dr .= '<tr>';
$data_dr .= '<td style="background-color: #f3e3fa;" colspan="3"><b>Total<b></td>';
if ($bank_total_dr != 0)
    $data_dr .= '<td style="background-color: #f3e3fa;" class="cl5"><b>'.number_format($bank_total_dr, 2).'</b></td>';
else
     $data_dr .= '<td style="background-color: #f3e3fa;" class="cl5"><b>--</b></td>';
if ($cash_total_dr != 0)
	  $data_dr .= '<td style="background-color: #f3e3fa;" class="cl6"><b>'.number_format($cash_total_dr, 2).'</b></td></tr>';
else
     $data_dr .= '<td style="background-color: #f3e3fa;" class="cl6"><b>--</b></td></tr>';
...
$numrows = $conn->query("SELECT je.journal_date, jt.journal_id, jt.group_id, je.description, al.name, je.voucher_number, jt.amount, jt.account_id, jt.group_id, je.payee_name FROM `journal_entries` je inner join `journal_items` jt on je.id = jt.journal_id inner join account_list al on al.id = jt.account_id WHERE dli_type = 'DLI' and account_id >= 51 and account_id <= 52 and journal_date >= '$start' and journal_date <= '$end' order by journal_date asc;")->num_rows;
	  ...
	  if ($numrows > 0){	
			...
			if (($row['debit_count'] > 1) && ($row['credit_count'] == 1) && ($row['group_id'] == 4)) {
				...
				while($row_j = $journals_item->fetch_assoc()){
					 ...
					 if ($row['name'] == 'Bank') {
							...
							$data_dv .= '<td class="cl6" style="padding: 3px; text-align: right;">--</td></tr>';
							$bank_total_cr += $row_j['amount'];
						}
					  if ($row['name'] == 'Cash') {
							$data_dv .= '<td class="cl4" style="padding: 3px; text-align: right;">--</td>';
							$data_dv .= '<td class="cl6" style="padding: 3px; text-align: right;">'.format_num($row_j['amount']).'</td></tr>';
			...
			if (($row['debit_count'] == 1) && ($row['credit_count'] > 1) && ($row['group_id'] == 4)) {
				...
				while($row_j = $journals_item->fetch_assoc()){
					 ...
					 if ($row['name'] == 'Bank') {
					  ...
					  if ($row['name'] == 'Cash') {
							$data_dv .= '<td class="cl4" style="padding: 3px; text-align: right;">--</td>';
							$data_dv .= '<td class="cl6" style="padding: 3px; text-align: right;">'.format_num($dbt_amt).'</td></tr>';
		...
		if (($bank_total_cr != 0) && ($cash_total_cr != 0)) {
			$data_dv .= '<tr><td class="cl4" style="padding: 3px; height: 24px;">'.date("d-m-Y", strtotime($end)).'</td>';
			$data_dv .= '<td class="cl1" colspan="2" style="padding: 3px;">Total Expenditure </td>';
			$data_dv .= '<td class="cl5" style="padding: 3px;">'.number_format($bank_total_cr, 2).'</td>';
			$data_dv .= '<td class="cl6" style="padding: 3px;">'.number_format($cash_total_cr, 2).'</td></tr>';
			$counter_row_credit = $counter_row_credit + 1;
		} else if ($bank_total_cr != 0) {
			$data_dv .= '<tr><td class="cl4" style="padding: 3px; height: 24px;">'.date("d-m-Y", strtotime($end)).'</td>';
			$data_dv .= '<td class="cl1" colspan="2" style="padding: 3px;">Total Expenditure </td>';
			$data_dv .= '<td class="cl5" style="padding: 3px;">'.number_format($bank_total_cr, 2).'</td>';
			$data_dv .= '<td class="cl6" style="padding: 3px;">--</td></tr>';
			$counter_row_credit = $counter_row_credit + 1;
		} else if ($cash_total_cr != 0) {
			$data_dv .= '<tr><td class="cl4" style="padding: 3px; height: 24px;">'.date("d-m-Y", strtotime($end)).'</td>';
			$data_dv .= '<td class="cl1" colspan="2" style="padding: 3px;">Total Expenditure </td>';
		...
		if ($bank_balance != 0) {
			...
			$data_dv .= '<td class="cl5" style="padding: 3px;">'.number_format($bank_balance, 2).'</td>';
			$data_dv .= '<td class="cl6" style="padding: 3px;">--</td></tr>';
			$counter_row_credit = $counter_row_credit + 1;
		}
		if ($cash_balance != 0) {
			...
			$data_dv .= '<td class="cl1" colspan="2" style="padding: 3px;">Closing Balance C/D </td>';
			$data_dv .= '<td class="cl5" style="padding: 3px;">--</td>';
			$data_dv .= '<td class="cl6" style="padding: 3px;">'.number_format($cash_balance, 2).'</td></tr>';
			$counter_row_credit = $counter_row_credit + 1;
		} */
...
while($diff > 0){
	...
	$data .= '<td class="cl2" style="text-align: center;" style="padding: 3px;"></td>';
	$data .= '<td class="cl5" style="text-align: center;"  style="padding: 3px;"></td>';
	$data .= '<td class="cl6" style="padding: 3px;"></td></tr>';
	$diff = $diff - 1;
}


$data_cr .= '<tr>';
$data_cr .= '<td style="background-color: #b7f7db;" colspan="3"><b>Total<b></td>';
if (($bank_total_cr + $bank_balance) != 0)
  $data_cr .= '<td style="background-color: #b7f7db;" class="cl5"><b>'.number_format(($bank_total_cr + $bank_balance), 2).'</b></td>';
else
    $data_cr .= '<td style="background-color: #b7f7db;" class="cl5"><b>--</b></td>';
if (($cash_total_cr + $cash_balance) != 0)
  $data_cr .= '<td style="background-color: #b7f7db;" class="cl6"><b>'.number_format(($cash_total_cr + $cash_balance), 2).'</b></td></tr>';
else
    $data_cr .= '<td style="background-color: #b7f7db;" class="cl6"><b>--</b></td></tr>';


$header = '<!--mpdf
<htmlpageheader name="letterheader">
		<div style="font-size: 10pt; text-align: center; padding-top: 1mm; font-family: nikosh; ">
		<h3 style="margin-bottom: 2px;">==== Cash Book (DLI)====</h3>
    ...
    <div class="column">
	  <table>
		...
		<tr>
		  ...
		  <th class="cl6" style="background-color: #f3e3fa;"><b>Cash</b></th>
		</tr>
		</thead>		  
		<tbody>
		'.$data.'
	...
	<div class="column">
	  <table>
		...
		<tr>
		  ...
		  <th class="cl6" style="background-color: #b7f7db;"><b>Cash</b></th>
		</tr>
		</thead>		  
		<tbody>
		'.$data_dv.'
