...
if (empty($_POST['account_id'])) {  
	...
	while($a_list = $acc_list->fetch_assoc()){
		...
		if ($openningNumRows > 0) {	
			while($rows = $openningBalance->fetch_assoc()){
				...
				if ($rows["amount"] < 0) {
				  ...
				  $data .= '<td class="cl5">'.format_num($balance*-1).'</td></tr>';
				}
			}
		}
	  ...
	  if (($numrows > 0) || ($openningNumRows > 0)) {	
		while($row = $journals->fetch_assoc()){
			...
			if ((($debit != 0) || ($credit != 0)) && ($account_id != 99)) {
					$data .= '<tr><td class="cl4" colspan="3" style="padding: 3px; background-color: #f3e3fa;"><b>Total</b></td><td class="cl3" style="background-color: #f3e3fa;"><b>'.format_num($debit).'</b></td><td class="cl3" style="background-color: #f3e3fa;"><b>'.format_num($credit).'</b></td><td class="cl5" style="background-color: #f3e3fa;"><b>'.format_num($balance).'</b></td></tr>';
				}
